...
//클래스는 System.Serializable이라는 어트리뷰트(Attribute)를 명시
//해야 인스펙터 뷰에 노출
[System.Serializable]
...
public class PlayerCtrl : MonoBehaviour {
    ...
    void Start () {
        //스크립트가 실행된 후 처음 수행되는 Start함수에서 Transform 컴포넌트 할당
    ...
    void Update () {
        ...
        //Debug.Log("h=" + h.ToString()); // String화 해서 표시할때 자주사용
        ...
        tr.Translate(moveDir.normalized * moveSpeed * Time.deltaTime, Space.Self);//플레이어 위치를 지정
        ...
        {
            anim.CrossFade(playerAnim.idle.name, 0.3f); // 정지시 대기
        }
    }
}
