...
public class MoveAgent : MonoBehaviour {
    //순찰 지점들을 저장하기 위한 List변수
    public List<Transform> wayPoints;
    //다음 순찰 지점의 배열의 Index
    public int nextIdx;

    private readonly float patrolSpeed = 1.5f;
    private readonly float treaceSpeed = 4.0f;
    //회전할 때의 속도를 조절하는 계수
    ...
    //현재 EnemyAI에서는 적캐릭터의 애니메이션 제어부분이 포함되어 있다.
    //MoveAgent는 목적지 설정만 포함
    //EnemyAI에서 적 캐릭터가 걸을지 달려갈지를 판단하려면 
    //NavMeshAgent의 속도 값이 필요하다. 그래서 아래와 같이 프로퍼티를 정의한다.
    ...
    void Start () {
        ...
        //MoveWayPoint();
        //patrolling 프로퍼티를 true로 줘야만 get/set 프로퍼티 동작이 정상 수행
